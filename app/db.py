...
async def ensure_indexes(db: AsyncDatabase) -> None:
    ...
    await db.admins.create_index("account_id")
    await db.apps.create_index("account_id")
    await db.activity_log.create_index([("account_id", ASCENDING), ("created_at", DESCENDING)])
    await db.invitations.create_index("token_hash", unique=True)
    await db.invitations.create_index([("account_id", ASCENDING), ("email", ASCENDING)])
    await db.email_tokens.create_index("token_hash", unique=True)
    await db.api_keys.create_index("key_hash", unique=True)
    await db.reviews.create_index([("app_id", ASCENDING), ("user_id", ASCENDING)], unique=True)
    await db.reviews.create_index([("app_id", ASCENDING), ("updated_at", DESCENDING)])
    await db.reviews.create_index("reported_at", sparse=True)
    await db.app_reports.create_index([("status", ASCENDING), ("created_at", DESCENDING)])
    await db.api_keys.create_index("account_id")
    await db.versions.create_index([("status", ASCENDING), ("scheduled_at", ASCENDING)])
    ...
    await db.app_views.create_index([("timestamp", DESCENDING)])
    await db.app_views.create_index([("app_id", ASCENDING), ("visitor", ASCENDING), ("timestamp", DESCENDING)])
    await db.installations.create_index([("device", ASCENDING), ("app_id", ASCENDING)], unique=True)
    await db.installations.create_index([("app_id", ASCENDING), ("last_seen", DESCENDING)])
