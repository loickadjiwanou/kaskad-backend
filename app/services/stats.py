...
def _range_filter(start: datetime | None, end: datetime | None, app_id: ObjectId | None, version_id: ObjectId | None) -> dict:
    end = end or now()
...
async def downloads_timeseries(
    ...
    pipeline = [
        {"$match": _range_filter(start, end, app_id, version_id)},
...
async def downloads_by(db: AsyncDatabase, field: str, start, end, app_id=None) -> list[dict]:
    pipeline = [
        {"$match": _range_filter(start, end, app_id, None)},
...
async def top_apps(db: AsyncDatabase, start, end, limit: int = 10) -> list[dict]:
    pipeline = [
        {"$match": _range_filter(start, end, None, None)},
...
async def raw_downloads(db: AsyncDatabase, start, end, app_id=None):
    """Itérateur des téléchargements (export CSV)."""
    cursor = db.download_stats.find(_range_filter(start, end, app_id, None)).sort("timestamp", 1)
